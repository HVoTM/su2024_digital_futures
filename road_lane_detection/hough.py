...
def lane_lines(image, lines):
    ...
    if left_lane is not None and right_lane is not None:
        x_vanish = int(VanishingPoint(left_lane=left_lane, right_lane=right_lane))
    else:
        x_vanish = int(image.shape[1] // 2)
    ...
    return left_line, right_line, x_vanish

def VanishingPoint(left_lane, right_lane):
    """
    Utility function to calculate the converging point (a virtual intersection point) of two
    parallel lines from the 3D world into a 2D image

    Given slope and intercept for each one, use a equivalent equation to find the endpoint

    Parameters:
    >> left_lane: array of weighted [slope, intercept]
    >> right_lane: similar to left lane but for right lane
    """
    return (right_lane[1] - left_lane[1])/ (left_lane[0] - right_lane[0])    
...
def Fit_Polynomial(lines, image_width):
    ...
    def fit_line(lines):
        ...
        for x1, y1, x2, y2 in lines:
            xs.extend([x1, x2])
            ys.extend([y1, y2])
        coefficients = np.polyfit(xs, ys, 1)  # Fit a curved line with polynomial degree of 2
        # DEBUG
        print(coefficients)
...
def FramePreProcessing(frame:np.ndarray) -> np.ndarray:
    """
...
def LaneDetection(image: np.ndarray) -> np.ndarray:
    ...
    lines = FramePreProcessing(image)
...
def LeftRightLaneDetection(image: np.ndarray) -> np.ndarray:
    ...
    if lines is not None:
        ...
        left_line, right_line, x_vanish = lane_lines(image, lines)
        print("x-vanishing point is: ", x_vanish)
        result = DrawLines(image, (left_line, right_line))
        
        # draw the center point for the vehicle to align itself in the middle of the lane
        cv.circle(result, (x_vanish, image.shape[0] - 10), 5, (0, 50, 255), 5)
        return result
    return image

def LaneDetectionPolyFit(image:np.ndarray) -> np.ndarray:
    "Lane Detection Processing Pipeline for image input"
    lines = FramePreProcessing(frame=image)
...
def ShowProcessedWebcam(source:Union[int, str]= 0) -> None:
    ...
    while True:
        ...
        # Process the frame
        processed_frame = LeftRightLaneDetection(frame)
