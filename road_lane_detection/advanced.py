...
1. PRESETTING THE NUMBERS: Compute the camera calibration matrix and distortion coefficients.

2. Apply a distortion correction to raw images.

3. Use color transforms, gradients, etc., to create a thresholded binary image.

4. Apply a perspective transform to generate a “bird-eye view” of the image.
...
from misc import calculate_runtime
...
BOT_W, TOP_W, TOP_H, BOT_H = 0.92, 0.34, 0.5, 0.87
OFFSET = 0.25
...
def UnpackPickle() -> Tuple:
    with open('./camera_cal/calibration_pickle.p', 'rb') as f:
        data = pickle.load(f)
...
# wrapper function calculate runtime for this image processing pipeline for road lane detection
@calculate_runtime
...
def Preprocess2(image: np.ndarray, mtx, dist) -> np.ndarray:
    """ 
...
def WebcamLaneDetection(source, mtx, dist):
    ...
    while True:
        ...
        # Process the frame
        processed_frame = Preprocess2(frame, mtx, dist)
...
def LaneDetectionbyFrame(image: np.ndarray, mtx, dist):
    ...
    processed = Preprocess2(image=image, mtx=mtx, dist=dist)
    return processed
